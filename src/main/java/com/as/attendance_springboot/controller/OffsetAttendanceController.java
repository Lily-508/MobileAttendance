...
public class OffsetAttendanceController extends BaseController {
    ...
    public ResponseEntity<DataResult<List<OffsetAttendance>>> getOffsetAttendance(
            @RequestParam(required = false) Integer oaId,
    ...
    @ApiResponses({@ApiResponse(code = 200, message = "新建成功", response = BaseResult.class),
            ...
            @ApiResponse(code = 400, message = "补卡事务开始或结束时间不能为空", response = BaseResult.class)})
    public ResponseEntity<BaseResult> setOffsetAttendance(
            @NotNull @Valid @RequestBody OffsetAttendance offsetAttendance,
    ...
    public ResponseEntity<BaseResult> updateOffsetAttendance(@NotNull @Valid @RequestBody OffsetAttendance offsetAttendance,
        ...
        if (isErrorForeignId(offsetAttendance)) {
            ...
            result = super.updateModelBySingle(offsetAttendance.getOaId(),
                                               offsetAttendanceService,
                                               offsetAttendance,
                                               bindingResult);
            //补卡事务审核通过
            if (result.getCode() == 200 && offsetAttendance.getResult() == AuditType.AGREE) {
                LambdaUpdateWrapper<RecordAttendance> updateWrapper = new LambdaUpdateWrapper<>();
                updateWrapper.set(RecordAttendance::getRResult, RecordResultType.SUCCESS)
                             .eq(RecordAttendance::getRId, offsetAttendance.getRId());
                recordAttendanceService.update(updateWrapper);
            }
        }
        return ResponseEntity.status(result.getCode()).body(result);
    }
    ...
    @ApiResponses({@ApiResponse(code = 200, message = "删除成功", response = BaseResult.class),
            @ApiResponse(code = 400, message = "对应事务不存在或已审核通过", response = BaseResult.class),
    ...
    private boolean isErrorForeignId(OffsetAttendance offsetAttendance) {
        ...
        return staff == null || reviewer == null || recordAttendance == null ||
                reviewer.getSRight() != StaffRight.LEADER;
    }
}
